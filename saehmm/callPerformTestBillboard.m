function [] = callPerformTestBillboard(multi_res,pcp_pretrain,zeroMaskFrac,noisetype,vari,sparsity,sparsityPenalty,L2weightPenalty,dropout,id,shrinkfactor,layer1,layer2,layer3,layer4,layer5,layer6)
    ...
    [ffts_train,gts_full,gts_train] =loadTrainSAEBeatles(shrinkfactor,multi_res,id); %loadTrainSAEBillboard(shrinkfactor,multi_res,id);%%loadTrainSAEBillboard(shrinkfactor,0,'a');
...
%sparsityPenalty = varargin{4};
    if(strcmp(noisetype,'isoGauss'))
        [nn,inits,transitions]= trainSAEHMMBillboardIsoGauss(ffts_train,gts_train,gts_full,layer1,layer2,layer3,layer4,layer5,layer6, multi_res, pcp_pretrain,vari,zeroMaskFrac,sparsity,dropout,sparsityPenalty,L2weightPenalty);
    else
        [nn,inits,transitions]= trainSAEHMMBillboard(ffts_train,gts_train,gts_full,layer1,layer2,layer3,layer4,layer5,layer6, multi_res, pcp_pretrain,zeroMaskFrac,sparsity,dropout,sparsityPenalty,L2weightPenalty);
    end  
    ffts_train = [];
    gts_full = [];
    gts_train = [];
