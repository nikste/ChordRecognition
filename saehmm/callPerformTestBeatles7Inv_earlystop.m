function [] = callPerformTestBeatles7Inv_earlystop(multi_res,pcp_pretrain,zeroMaskFrac,noisetype,vari,sparsity,sparsityPenalty,L2weightPenalty,dropout,id,shrinkfactor,layer1,layer2,layer3,layer4,layer5,layer6)
    ...
    [ffts_train,gts_full,gts_train] =loadTrainSAEBeatles7Inv(shrinkfactor,multi_res,id);%loadTrainSAEBeatles(shrinkfactor,multi_res,id); %%%loadTrainSAEBillboard(shrinkfactor,0,'a');
...
%sparsityPenalty = varargin{4};
    if(strcmp(noisetype,'isoGauss'))
        [nn,inits,transitions]= trainSAEHMMBillboardIsoGauss7Inv_earlystop(ffts_train,gts_train,gts_full,layer1,layer2,layer3,layer4,layer5,layer6, multi_res, pcp_pretrain,vari,zeroMaskFrac,sparsity,dropout,sparsityPenalty,L2weightPenalty);
    else
        [nn,inits,transitions]= trainSAEHMMBillboard7Inv_earlystop(ffts_train,gts_train,gts_full,layer1,layer2,layer3,layer4,layer5,layer6, multi_res, pcp_pretrain,zeroMaskFrac,sparsity,dropout,sparsityPenalty,L2weightPenalty);
    end  
    ffts_train = [];
    gts_full = [];
    gts_train = [];
    
     [gt_test,ffts_test_no_preprocessing] = loadTestSAEHMM7Inv(id);%loadTestSAEHMM(id);
     [ gt_test_lab ] = loadLabTestBeatles7Inv( id);%loadLabTest(id);
