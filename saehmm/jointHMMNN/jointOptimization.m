function [ mus,sigmas,mixmats,nn ] = jointOptimization( ffts_train,gts_train, mus,sigmas,mixmats,nn )
...
for i=1:max_iter
    ...
    disp('computing emission probabilities of hmm states')
    ...
    tic
    batchsize = 1;
    for t = 1:batchsize:size(pcps_train,1)-1000
        ...
        %batchmode!
        bt_s_big = zeros(12,batchsize);
        BT_big = zeros(batchsize,1);
        ...
        for b=t:t+batchsize-1
            ...
            %[B, B2] = mixgauss_prob(pcps_test{song}', mus, sigmas, mixmats);
            
            %[alpha, beta, gamma, loglik, xi_summed, gamma2] = fwdback(init_state_distrib,transmat, obslik, varargin)
    ...
    %         bt_s2 = zeros(12,1);
            ...
            for k = 1:no_comps
                ...
                x = current_batch(b-t+1,:);%pcps_train(b,:);
                m = mus(:,state,k)';
                co = sigmas(:,:,state,k);
                w = mixmats(state,k);
                
                %"target"
                targets(b-t+1,:) = targets(b-t+1,:) + w * m;
                
                inv_co = inv(co);
                BT = BT + w/(sqrt((2*pi))^12 * sqrt(det(co))) * exp(-1/2 *(x - m) * inv_co * ( x- m)');
                for j = 1:size(bt_s,1)
                    fac = 0;
                    for l=1:size(co,1)
                        fac = fac + inv_co(j,l) * (m(l) - x(l));
                    end
                    bt_s(j) = bt_s(j) +  w/(sqrt((2*pi))^12 * sqrt(det(co))) *fac* exp(-1/2 *(x - m) * inv_co * ( x- m)');    
                end
    %             act = w * mvnpdf(x,m,co);
