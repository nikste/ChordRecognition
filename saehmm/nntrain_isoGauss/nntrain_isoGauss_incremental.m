function [nn, L]  = nntrain_isoGauss_incremental(sae,train_layer_num, opts, id, shrinkfactor, is_unsupervised)
...
%% load blacklist,
blacklistfile = strcat('/media/nikste/moarspace/masterthesis/blacklist_',num2str(id));
...
function [ffts_loaded,gts_loaded] = load_trainfiles_local_parallel(trainfile_local)
    fftfoldername = '/media/nikste/moarspace/masterthesis/data/fft';
...
function nnupdatefigures_incremental(nn,fhandle,L,opts,i)
...
%    plotting
    ...
    if strcmp(nn.output,'softmax')  %also plot classification error
        ...
        set(p1, 'Xlim',[0,i + 1])%opts.numepochs + 1])
        ...
        set(p2, 'Xlim',[0,i + 1])%opts.numepochs + 1])
        ...
        set(gca, 'Xlim',[0,i + 1])%opts.numepochs + 1])
