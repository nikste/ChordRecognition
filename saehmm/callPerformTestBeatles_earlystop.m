function [] = callPerformTestBeatles_earlystop(multi_res,pcp_pretrain,zeroMaskFrac,noisetype,vari,sparsity,sparsityPenalty,L2weightPenalty,dropout,id,shrinkfactor,layer1,layer2,layer3,layer4,layer5,layer6)
    ...
    [ffts_train,gts_full,gts_train] =loadTrainSAEBeatles(shrinkfactor,multi_res,id);%loadTrainSAEBeatles(shrinkfactor,multi_res,id); %%%loadTrainSAEBillboard(shrinkfactor,0,'a');
...
%sparsityPenalty = varargin{4};
    ...
    gts_train = [];
     ...
     [gt_test,ffts_test_no_preprocessing] = loadTestSAEHMMBeatles(id);%loadTestSAEHMM(id);
     [ gt_test_lab ] = loadLabTestBeatles( id);%loadLabTest(id);
