function [nn] = callPerformTestBillboard_earlystop_incremental2(multi_res,pcp_pretrain,zeroMaskFrac,noisetype,vari,sparsity,sparsityPenalty,L2weightPenalty,dropout,id,shrinkfactor,layer1,layer2,layer3,layer4,layer5,layer6)
    ...
    % TODO load only gts_full 
    %[ffts_train,gts_full,gts_train] =loadTrainSAEBillboard(shrinkfactor,multi_res,id);%loadTrainSAEBeatles(shrinkfactor,multi_res,id); %%%loadTrainSAEBillboard(shrinkfactor,0,'a');
...
%sparsityPenalty = varargin{4};
    if(strcmp(noisetype,'isoGauss'))
        [nn,inits,transitions]= trainSAEHMMBillboardIsoGauss_earlystop_incremental2(id,layer1,layer2,layer3,layer4,layer5,layer6, layer6, multi_res, pcp_pretrain,vari,zeroMaskFrac,sparsity,dropout,sparsityPenalty,L2weightPenalty);%trainSAEHMMBillboardIsoGauss_earlystop_incremental(id,gts_full,layer1,layer2,layer3,layer4,layer5,layer6, multi_res, pcp_pretrain,vari,zeroMaskFrac,sparsity,dropout,sparsityPenalty,L2weightPenalty);
    else
    ...
    gts_train = [];
     ...
     [gt_test,ffts_test_no_preprocessing] = loadTestSAEHMMBillboard(id);%loadTestSAEHMM(id);
     [ gt_test_lab ] = loadLabTestBillboard( id);%loadLabTest(id);
