function res = testString2Chord(basefolder)
...
for i=3:length(listingwav)
    ...
    for l = 1:length(labdata)
        ...
        if(0 < sum(ismember(strs,num2str(string2chord(aux{3})))))
            %do nothing
        else
            strs{length(strs)+1} = num2str(string2chord(aux{3}));
%         for j=1:3
%             
...
% 
%     for g = 1:25
%         if(0 < sum(ismember(strs,num2str(g))))
%             %pass
%         else
%             not_in_there = g
%         end
